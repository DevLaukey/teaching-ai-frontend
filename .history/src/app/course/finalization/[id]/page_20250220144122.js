...
const CourseFinalization = () => {
  ...
  const exportToPPTX = async () => {
    ...
    try {
      ...
      let content = `${courseData.title.toUpperCase()}\n`;
      ...
      a.href = url;
      a.download = `${courseData.title.replace(/\s+/g, "_")}_presentation.pptx`;
  ...
  const exportToDOCX = async () => {
    ...
    try {
      ...
      content += `Interactive Activities: ${
        formattedSlides.filter((s) => s.interactiveActivity).length
      }\n\n`;
      content += "=".repeat(60) + "\n\n";

      formattedSlides.forEach((slide) => {
        content += `Chapter ${slide.slideNumber}: ${slide.title}\n\n`;
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `${courseData.title.replace(/\s+/g, "_")}_document.docx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      formattedSlides.forEach((slide) => {
        // Page header
        content += `Page ${slide.slideNumber}\n`;
        content += "-".repeat(20) + "\n\n";
        ...
        // Interactive Activity in a highlighted box
        if (slide.interactiveActivity) {
          content += "*".repeat(40) + "\n";
          content += "Interactive Activity:\n";
          content += `${slide.interactiveActivity}\n`;
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `${courseData.title.replace(/\s+/g, "_")}_course.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
