...
const ContentPreview = () => {
  ...
  const processSlides = (rawSlides) => {
    ...
    for (const slide of rawSlides) {
      ...
      // Process slide based on content type - handle new format with markdown headings
      if (slide.title.startsWith("### Slide") || slide.title.includes("**Slide")) {
        // New slide starts
        if (currentSlide.id) {
          processedSlides.push({ ...currentSlide });
        }
        currentSlide = {
          id: slide.id,
          // Extract the title from either the title or content
          title: slide.title.startsWith("### Slide") 
            ? slide.title.replace(/^### Slide \d+:\s*/, "") 
            : slide.content.replace("Title: ", ""),
          mainContent: "",
          examples: "",
          interactiveActivity: "",
      ...
      } else if (slide.content && slide.content.includes("**Content:**")) {
        // Parse the content section which may include multiple parts
        const content = slide.content;
        
        // Extract main content
        const contentMatch = content.match(/\*\*Content:\*\*\s*([\s\S]*?)(?=\*\*Examples:|$)/);
        if (contentMatch && contentMatch[1]) {
          currentSlide.mainContent = contentMatch[1].trim();
        }
        
        // Extract examples
        const examplesMatch = content.match(/\*\*Examples:\*\*\s*([\s\S]*?)(?=\*\*Activity:|$)/);
        if (examplesMatch && examplesMatch[1]) {
          currentSlide.examples = examplesMatch[1].trim();
        }
        
        // Extract activity
        const activityMatch = content.match(/\*\*Activity:\*\*\s*([\s\S]*?)$/);
        if (activityMatch && activityMatch[1]) {
          currentSlide.interactiveActivity = activityMatch[1].trim();
        }
      } else if (slide.title.startsWith("Content:") || slide.title.includes("**Content:**")) {
        currentSlide.mainContent +=
          (currentSlide.mainContent ? "\n" : "") +
          slide.title.replace(/Content:|^\*\*Content:\*\*\s*/, "").trim();
        if (slide.content) {
          currentSlide.mainContent += "\n" + slide.content;
        }
      } else if (slide.title === "Examples:" || slide.title.includes("**Examples:**")) {
        currentSlide.examples = slide.content || slide.title.replace(/Examples:|^\*\*Examples:\*\*\s*/, "").trim();
      } else if (slide.title.startsWith("Interactive activity:") || 
                 slide.title.startsWith("Activity:") || 
                 slide.title.includes("**Activity:**")) {
        currentSlide.interactiveActivity = (slide.content || "").trim() || 
          slide.title.replace(/Interactive activity:|Activity:|^\*\*Activity:\*\*\s*/, "").trim();
      } else if (!slide.title.includes("**Slide") && !slide.title.startsWith("### Slide")) {
